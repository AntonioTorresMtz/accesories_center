...
if ($encontrado) {
    ...
    $query = "INSERT INTO `ventas` (`id_venta`, `id_producto`, `marca`, `fecha`, `id_tipo`, `cantidad`,
     `precio`, `descuento`, `nombre_modelo`)
      VALUES (NULL, '3', '$marca', current_timestamp(), NULL, '$cantidad', '$precio', '$descuento', '$nombre_modelo')";
    ...
    if(!$resultado){
        ...
        echo 'Exito mica <br>' ;
        $query2 = "UPDATE `micas100d` SET `cantidad` = cantidad -'$cantidad' WHERE `micas100d`.`id_mica100d` = '$id_modelo'";
        $resultado2 = mysqli_query($conn, $query2);
        if(!$resultado2){
            echo 'Error actualizar cantidad <br>';
            $_SESSION['error_ventaMica100d'] = "Mica guardada";
            header("Location: ../ventaMenu_mica100d.php");
            exit(); 
        } else{
            $_SESSION['exito_ventaMica100d'] = "Mica guardada";
            header("Location: ../ventaMenu_mica100d.php");
            exit(); 
        }     
    } 
